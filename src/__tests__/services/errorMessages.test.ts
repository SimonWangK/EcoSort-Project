...
describe('friendlyFirebaseError', () => {
  ...
  test('returns friendly message when "API key" appears in message', () => {
    ...
    expect(msg.toLowerCase()).toContain('account setup');
  });
...
describe('permissionErrorMessage', () => {
  ...
  test('location message mentions manual council selection', () => {
    const msg = permissionErrorMessage('location');
    expect(msg.toLowerCase()).toContain('council');
  });

  test('notification message mentions app usage', () => {
