...
import { classifyBarcodeScan, matchCouncilRule, normalizeText } from '../../services/ruleMatcher';
...
describe('classifyBarcodeScan', () => {
  test('matches QR payload text to active council rule', () => {
    const result = classifyBarcodeScan('item=battery', 'Yarra Council');
    expect(result?.item).toBe('battery');
    expect(result?.council).toBe('Yarra Council');
  });

  test('matches known product barcode value', () => {
    const result = classifyBarcodeScan('9340001000035', 'Merri-bek City Council');
    expect(result?.item).toBe('glass bottle');
    expect(result?.council).toBe('Merri-bek City Council');
  });

  test('does not guess when barcode is unknown', () => {
    const result = classifyBarcodeScan('9999999999999', 'Yarra Council');
