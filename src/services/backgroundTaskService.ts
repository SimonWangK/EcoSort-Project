 ...
 * BGTaskScheduler. The task now performs the same lightweight local rule-cache
 * refresh used by the foreground path, then persists the updated timestamp.
 */

import * as TaskManager from 'expo-task-manager';
import * as BackgroundTask from 'expo-background-task';
import { loadAuthSession, loadSettings, saveSettings } from './storageService';
import { needsCouncilSync, syncCouncilRules } from './councilSyncService';
import { isLocalAuthSession, syncCouncilMetaToFirestore } from './firebaseService';
...
async function performCouncilRuleBackgroundSync(): Promise<void> {
  const settings = await loadSettings();
  if (!needsCouncilSync(settings)) return;

  const { updatedSettings, council } = await syncCouncilRules(settings);
  await saveSettings(updatedSettings);

  const session = await loadAuthSession();
  if (session && !isLocalAuthSession(session)) {
    await syncCouncilMetaToFirestore(session, council);
  }
}
...
function ensureTaskDefined(): void {
  ...
  TaskManager.defineTask(COUNCIL_RULE_SYNC_TASK, async () => {
    try {
      await performCouncilRuleBackgroundSync();
