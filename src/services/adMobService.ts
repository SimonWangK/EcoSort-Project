 ...
 * initialization there. In a development/EAS build it initializes Google
 * Mobile Ads and exposes real test-unit loading/showing helpers.
 */
...
function makeInterstitial(platform: 'android' | 'ios') {
  const module = loadGoogleMobileAds();
  if (!module?.InterstitialAd || !module?.AdEventType) return null;

  return module.InterstitialAd.createForAdRequest(getInterstitialAdUnitId(platform), {
...
export async function preloadInterstitialAd(platform: 'android' | 'ios' = Platform.OS === 'ios' ? 'ios' : 'android'): Promise<boolean> {
  ...
  const module = loadGoogleMobileAds();
  const ad = makeInterstitial(platform);
  if (!ad || !module?.AdEventType) return false;
  ...
  try {
    return await new Promise<boolean>((resolve) => {
      ...
      const unsubscribeLoaded = ad.addAdEventListener(module.AdEventType.LOADED, () => finish(true));
      const unsubscribeError = ad.addAdEventListener(module.AdEventType.ERROR, () => finish(false));
      ad.load();
    });
  } catch {
    return false;
...
export async function showInterstitialAd(platform: 'android' | 'ios' = Platform.OS === 'ios' ? 'ios' : 'android'): Promise<boolean> {
  ...
  try {
    return await new Promise<boolean>((resolve) => {
      ...
      const unsubscribeClosed = ad.addAdEventListener(module.AdEventType.CLOSED, () => finish(true));
      const unsubscribeError = ad.addAdEventListener(module.AdEventType.ERROR, () => finish(false));
      ad.load();
    });
  } catch {
