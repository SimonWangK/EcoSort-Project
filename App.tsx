...
export default function App() {
  ...
  const restoreCloudData = async (auth: AuthSession, baseSettings: AppSettings, baseHistory: ScanHistoryEntry[]) => {
    if (isLocalAuthSession(auth)) return;

    try {
      const [remoteSettings, remoteHistory] = await Promise.all([
        loadSettingsFromFirestore(auth),
        loadHistoryFromFirestore(auth),
      ]);

      const nextSettings = remoteSettings ? { ...baseSettings, ...remoteSettings } : baseSettings;
      const nextHistory = mergeScanHistories(baseHistory, remoteHistory);

      setSettings(nextSettings);
      setHistory(nextHistory);
      await Promise.all([saveSettings(nextSettings), saveHistory(nextHistory)]);

      try {
        await initSQLiteDB();
        await Promise.all(nextHistory.map(insertScanEntry));
      } catch {
        // AsyncStorage remains the fallback store when SQLite is unavailable.
      }
    } catch {
      // Cloud restore is best-effort. Local history/settings continue to work.
    }
  };
  ...
  useEffect(() => {
    const boot = async () => {
      try {
        ...
        if (auth && !isLocalAuthSession(auth)) {
          await restoreCloudData(auth, storedSettings, durableHistory);
        }
      } catch {
        Alert.alert('Storage', 'EcoSort could not restore your saved data.');
      } finally {
        setBootReady(true);
      }
    };
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener('change', async (nextState) => {
      ...
      if (!sessionRef.current || isLocalAuthSession(sessionRef.current)) return; // local mode: no cloud sync
      if (!needsCouncilSync(settingsRef.current)) return; // recent sync — skip

      try {
        const { updatedSettings, council } = await syncCouncilRules(settingsRef.current);
        // Persist and update state through the normal channel
        setSettings(updatedSettings);
        await saveSettings(updatedSettings);
        await syncCouncilMetaToFirestore(sessionRef.current, council);
      } catch {
  ...
  const handleLogin = async (s: AuthSession) => {
    ...
    setSession(s);
    await restoreCloudData(s, settings, history);
  };
  ...
  const handleScheduleReminder = async () => {
    try {
      ...
      Alert.alert('Reminder scheduled', 'A reminder will appear shortly.');
    } catch (error) {
  ...
  const renderScreen = () => {
    switch (screen) {
      ...
      case 'search':   return <SearchScreen onSaveResult={handleSaveResult} settings={settings} />;
      case 'scan':     return <ScannerScreen onSaveResult={handleSaveResult} settings={settings} navigate={navigate} />;
